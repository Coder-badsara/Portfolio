...
function CertCard({ title, issuer, date, certUrl }) {
  return (
...
export default function Portfolio() {
  ...
  const handleSubmitMessage = async (e) => {
    ...
    if (!formData.name.trim() || !formData.email.trim() || !formData.message.trim()) {
      setSubmitStatus({ type: "error", message: "Please fill in all fields before sending." });
      return;
    }
      ...
      <section id="certifications" style={{ padding: "80px 5%", position: "relative", zIndex: 1 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16, marginTop: 36 }}>
            <CertCard
              title="Understanding Generative AI for Tech Leaders"
              issuer="LinkedIn Learning"
              date="July 2025"
              certUrl="https://drive.google.com/file/d/1Gau6PCAKLKDfjtvZqQS5G4HUTcFrRUKA/view?usp=sharing"
            />
            <CertCard
              title="Hacktoberfest Participation Certificate"
              issuer="Open Source Contribution — Hacktoberfest"
              date="October 2025"
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div className="contact-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40 }}>
            ...
            <div style={{ ...style.card, padding: "28px 26px" }}>
              ...
              <form onSubmit={handleSubmitMessage} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <input
                  name="name"
                  value={formData.name}
                ...
                {submitStatus.message && (
                  <p style={{
                    fontSize: 13,
                    color: submitStatus.type === "success" ? COLORS.accent : "#ff6b6b",
                    lineHeight: 1.4,
                  }}>
                    {submitStatus.message}
                  </p>
                ...
                <button
                  ...
                  onMouseEnter={e => e.currentTarget.style.opacity = "0.85"}
                  onMouseLeave={e => e.currentTarget.style.opacity = "1"}
                ><ArrowRight size={16} /> {isSending ? "Sending..." : "Send Message"}</button>
              </form>
            </div>
          </div>
        </div>
